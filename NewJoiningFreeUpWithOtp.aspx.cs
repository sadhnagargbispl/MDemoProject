...
public partial class NewJoiningFreeUpWithOtp : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //  QueryString: "PhN1"
                //===========================
                if (Request.QueryString["PhN1"] != null)
                {
                    string req1 = Request.QueryString["PhN1"].Replace(" ", "+");
                    string dec1 = Crypto.Decrypt(req1);
                    string[] rfAr1 = dec1.Split('/');

                    if (rfAr1.Length >= 2)
                    {
                        if (!string.IsNullOrEmpty(rfAr1[0]) && rfAr1[1] == "1")
                        {
                        ...
                        else if (!string.IsNullOrEmpty(rfAr1[0]) && rfAr1[1] == "2")
                        {
                            txtMobileNo.Text = rfAr1[0];
                            RbtnLegNo.SelectedIndex = 1;
                            RbtnLegNo.Enabled = false;
                    ...
                    if (Request.QueryString["PhN1"].Length > 0)
                ...
                {
                    string req2 = Request.QueryString["Email"].Replace(" ", "+");
                    string dec2 = Crypto.Decrypt(req2);
                    string[] rfAr2 = dec2.Split('/');

                    if (rfAr2.Length >= 2)
                    {
                        if (!string.IsNullOrEmpty(rfAr2[0]) && rfAr2[1] == "1")
                        {
                        ...
                        else if (!string.IsNullOrEmpty(rfAr2[0]) && rfAr2[1] == "2")
                        {
                            txtEMailId.Text = rfAr2[0];
                            RbtnLegNo.SelectedIndex = 1;
                            RbtnLegNo.Enabled = false;
                    ...
                    if (Request.QueryString["Email"].Length > 0)
    ...
    protected void CmdSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                if (txtFrstNm.Text.Trim() == "")
                {
                    ShowMsg("Please Enter Firm Name.");
                    return;
                }

                if (TxtAuthorizedPerson.Text.Trim() == "")
                {
                    ShowMsg("Please Enter Authorized Person Name.");
                    return;
                }
            ...
            if (Validt_SpnsrDtl("") == "OK")
            {
                ...
                if (isOk1 > 0)
                {
                    strQry = " insert into m_memberMaster(SessId,IdNo,CardNo,FormNo,KitId,UpLnFormNo,RefId,LegNo,RefLegNo,RefFormNo,"
                               ...
                               + "AadharNo,AadharNo2,AAdharNo3,AreaName,AreaCode,PostalPin,PostalStateCode,PostalAreaCode,PostalDistrict,PostalCity,PostalState,fld5,fld6,AuthorizedPName,RegType,RegNo)";
                    strQry += "Values(" + Session["SessID"] + ",0,0,0," + Session["Kitid"] + "," + Session["Uplnr"] + ",0," + iLeg + ",0,"
                           ...
                           + "'" + dblDistrict + "','" + dblTehsil + "','" + ddlState.SelectedItem.Text + "','D','WEB','" + ClearInject(TxtAuthorizedPerson.Text.ToUpper()) + "','" + RbCategory.SelectedValue + "','" + CbSubCategory.SelectedValue + "')";
